...
color Phong::phong_shading(hit_record& rec, Light& light, Camera& camera) {
	...
	color ambient_color = ka * rec.obj_material.get_diffuse_color();
	...
	if (RdotV >= 0)
	{
		...
		specular_color += (light.light_color * light.intensity * specular * ks * rec.obj_material.get_specular_color());
	}
